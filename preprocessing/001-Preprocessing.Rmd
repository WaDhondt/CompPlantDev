...
R Packages we will need, and checking whether necessary python packages for PacMAP are installed:
...
Other normalization method to explore: 

- normalization by deconvolution (account for compositional bias between cells due to DE)
...
```{r}
denyer_wt <- FindVariableFeatures(denyer_wt, selection.method = "vst", nfeatures=2000)
```
