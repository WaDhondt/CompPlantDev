...
signalheatmap <- pheatmap(t(scale(t(pairs_heatmapdata))), cluster_cols = F, annotation_row = row_meta, show_rownames = F, cutree_rows = 8)
```
```{r}
signal_modules <- data.frame(cluster = as.factor(cutree(signalheatmap$tree_row,  k = 8)))
...
Heatmap(t(scale(t(pairs_heatmapdata))),
